...
def fetch_flights_from_api(state):
    ...
    if not origin_iata or not dest_iata:
        print("[INFO] Missing origin/destination IATA codes, cannot query AviationStack.")
        return state
    ...
    try:
        ...
        if not api_url_map:
            print("[INFO] AviationStack returned no suitable passenger flights; leaving flights unchanged.")
            return state
        ...
        if state.flights:
            ...
            # Keep at most 8
            state.flights = fallback_flights[:8]

        return state

    except Exception as e:
        print("[WARNING] Error calling AviationStack:", str(e).encode('ascii', 'ignore').decode('ascii'))
